...
###
def compute_scale_factor(recipe: dict, target_weight: float) -> float:
    base_total = sum(recipe.get("ingredients", {}).values())
    return target_weight / base_total if base_total > 0 else 1.0

###
def scale_recipe_recursive(recipe: dict, factor: float) -> dict:
    scaled = {}

    # Scale ingredients
    scaled["ingredients"] = {
        k: v * factor for k, v in recipe.get("ingredients", {}).items()
    }

    # Keep instructions untouched
    scaled["instruction"] = recipe.get("instruction", [])

    # Scale subrecipes RECURSIVELY
    scaled_subrecipes = {}
    for name, sub in recipe.get("subrecipes", {}).items():
        scaled_subrecipes[name] = scale_recipe_recursive(sub, factor)

    scaled["subrecipes"] = scaled_subrecipes
